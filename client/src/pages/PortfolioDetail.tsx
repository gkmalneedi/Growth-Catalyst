...
export default function PortfolioDetail() {
  ...
  const currentIndex = portfolioWorks.findIndex(w => w.slug === slug);
  const nextWork = portfolioWorks[(currentIndex + 1) % portfolioWorks.length];
  ...
  const nextImg = imageMap[nextWork.coverImage] || "";

  return (
    <div className="min-h-screen bg-zinc-950 text-white overflow-x-hidden">
      <Navbar />

      {/* ── HEADER: back link + client name + headline (Miraki ISB style) ── */}
      <section className="pt-32 pb-12 bg-zinc-950">
        <div className="container mx-auto px-4 md:px-8">
          ...
          >
            ...
            {work.services.map(s => (
              <span key={s} className="text-xs px-3 py-1 rounded-full border border-white/10 text-zinc-400">
                {s}
              </span>
            ))}
          </motion.div>
      ...
      {/* ── RESULTS STATS (dark strip) ── */}
      <section className="py-20 bg-black border-y border-white/5">
        <div className="container mx-auto px-4 md:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-0">
            {work.results.map((stat, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 20 }}
        ...
        <div className="container mx-auto px-4 md:px-8">
          <div className="flex flex-col lg:flex-row gap-16 lg:gap-24">
            <div className="lg:w-1/3">
              ...
              <h2 className="text-3xl md:text-4xl font-heading font-bold text-zinc-900 leading-tight">
                About {work.client}
              </h2>
            </div>
          ...
          <div className="flex flex-col lg:flex-row gap-16 lg:gap-24">
            ...
            <div className="lg:w-2/3">
              ...
              <div className="flex flex-wrap gap-3">
                {work.services.map(s => (
                  <span key={s} className="px-4 py-2 rounded-full border border-white/10 bg-white/5 text-sm text-zinc-300">
                    {s}
                  </span>
                ))}
              </div>
        ...
        <div className="container mx-auto px-4 md:px-8">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8 mb-10">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-heading font-bold text-white leading-tight max-w-2xl">
                {nextWork.headline}
              </h2>
            </div>
