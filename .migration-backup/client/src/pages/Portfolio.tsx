...
export default function Portfolio() {
  const { data: portfolioWorks = [], isLoading } = useQuery<any[]>({
    queryKey: ["/api/portfolio"],
    staleTime: 5 * 60 * 1000,
  });

  const activeWorks = portfolioWorks.filter(w => w.active !== false);
        ...
        <div className="container mx-auto px-4 md:px-8">
          {isLoading ? (
            <div className="flex items-center justify-center py-24">
              <div className="text-zinc-400 text-lg">Loading portfolio...</div>
            </div>
          ) : (
      ...
      <section className="py-20 border-t border-white/8 bg-black">
        <div className="container mx-auto px-4 md:px-8 flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            ...
            <h2 className="text-3xl md:text-4xl font-heading font-bold text-white">Let's create your success story.</h2>
          </div>
          <Link href="/contact">
      ...
      <Link href={`/success-stories/${work.slug}`}>
        <div className="group cursor-pointer">
          <div className={`relative overflow-hidden rounded-2xl ${isFeature ? "aspect-[16/7]" : "aspect-[4/3]"}`}>
            {imgSrc ? (
              <img src={imgSrc} alt={work.client} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
            ) : (
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-400 flex items-end p-8">
              <div className="flex items-end justify-between w-full gap-4">
                <div className="translate-y-4 group-hover:translate-y-0 transition-transform duration-400">
                  <p className={`text-sm font-medium mb-2 ${categoryColors[work.category] || "text-brand-orange"}`}>{work.category}</p>
                  <h3 className="text-xl md:text-2xl font-heading font-bold text-white leading-tight max-w-xl line-clamp-2">{work.headline}</h3>
                </div>
          ...
          <div className="flex items-center justify-between mt-4 px-1">
            ...
            <span className="text-xs font-medium text-zinc-500 border border-white/8 px-3 py-1 rounded-full">{work.category}</span>
          </div>
        </div>
      </Link>
    </motion.div>
